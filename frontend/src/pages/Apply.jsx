...
function ScreeningResult({ application }) {
  ...
  return (
    <div className="card p-6">
      ...
      <p className="mt-4 text-slate-200">{s.summary}</p>
      ...
      {(s.matched_skills?.length > 0 || s.missing_skills?.length > 0) && (
        <div className="mt-5">
          <h3 className="text-xs font-bold uppercase tracking-wide text-slate-500">Skill match</h3>
...
export default function Apply() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <Link to={`/jobs/${id}`} className="text-sm text-brand-300 hover:underline">
        ← Back to job
      </Link>
      <div className="card mt-3 p-6">
        ...
        <form onSubmit={onSubmit} className="mt-6 space-y-4">
          ...
          <label
            className="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-white/10 bg-white/5 px-6 py-10 text-center transition hover:border-brand-400 hover:bg-brand-500/10"
